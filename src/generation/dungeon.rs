...
impl RoomCorridorGenerator {
    ...
    fn fill_unreachable_areas(&self, level: &mut Level) -> ThatchResult<()> {
        ...
        if reachable_tiles.is_empty() {
            ...
            return Err(ThatchError::GenerationFailed(format!(
                "Spawn position {:?} is not reachable",
                spawn_pos
            )));
        }
    ...
    fn add_stairs(
        ...
        if level.id < 25 {
            // Don't add stairs down on final level
            let stairs_down_pos =
                self.find_stairs_position_avoiding(level, rooms, false, stairs_up_pos, rng)?;
    ...
    fn find_stairs_position(
        ...
        Ok(Position::new(
            level.width as i32 / 2,
            level.height as i32 / 2,
        ))
    }
    ...
    pub fn generate_complete_dungeon(
        &self,
        config: &GenerationConfig,
        rng: &mut StdRng,
    ) -> ThatchResult<World> {
    ...
    fn generate_floor_with_stairs(
        ...
        let (stairs_up_pos, stairs_down_pos) = stair_positions
            .get(&floor_id)
            .cloned()
            .unwrap_or((None, None));
        ...
        while rooms.len()
            < (target_additional_rooms
                + if stairs_up_pos.is_some() { 1 } else { 0 }
                + if stairs_down_pos.is_some() { 1 } else { 0 })
...
impl Generator<Level> for RoomCorridorGenerator {
    fn generate(&self, config: &GenerationConfig, rng: &mut StdRng) -> ThatchResult<Level> {
        if self.generate_all_floors {
            ...
            return world.get_level(0).cloned().ok_or_else(|| {
                ThatchError::GenerationFailed(
                    "Failed to get first level from generated world".to_string(),
                )
            });
        }
        ...
        for level_id in 0..25 {
                ...
                (world.get_level(level_id), world.get_level(level_id + 1))
            {
                ...
                if let (Some(down_pos), Some(up_pos)) = (
                    current_level.stairs_down_position,
                    next_level.stairs_up_position,
                ) {
                    if down_pos != up_pos {
                        return Err(ThatchError::GenerationFailed(format!(
                            "Stair misalignment between levels {} and {}: down at {:?}, up at {:?}",
                            level_id,
                            level_id + 1,
                            down_pos,
                            up_pos
...
mod tests {
    ...
    fn test_fill_unreachable_areas() {
        ...
        assert_eq!(
            level.get_tile(spawn_pos).unwrap().tile_type,
            TileType::Floor
        );
        assert_eq!(
            level.get_tile(Position::new(5, 6)).unwrap().tile_type,
            TileType::Floor
        );
        assert_eq!(
            level.get_tile(Position::new(6, 5)).unwrap().tile_type,
            TileType::Floor
        );

        // Check that isolated area was filled with walls
        assert_eq!(
            level.get_tile(Position::new(2, 2)).unwrap().tile_type,
            TileType::Wall
        );
        assert_eq!(
            level.get_tile(Position::new(2, 3)).unwrap().tile_type,
            TileType::Wall
        );
    }
    ...
    fn test_stair_connectivity() {
            ...
            (level.stairs_up_position, level.stairs_down_position)
        {
            ...
            assert!(
                generator.has_path(&level, stairs_up, stairs_down).unwrap(),
            ...
            assert_ne!(
                ...
                "Stairs should be in different positions"
            );
        }
    }
    ...
    fn test_line_between_points() {
        ...
        let points = generator.line_between_points(Position::new(1, 5), Position::new(5, 5));
        ...
        // Test vertical line
        let points = generator.line_between_points(Position::new(3, 1), Position::new(3, 4));
        assert_eq!(points.len(), 4);
        assert!(points.contains(&Position::new(3, 1)));
        assert!(points.contains(&Position::new(3, 4)));

        // Test diagonal line
        let points = generator.line_between_points(Position::new(0, 0), Position::new(2, 2));
    ...
    fn test_stair_connection_creation() {
        ...
        // Create connection
        generator
            .create_stair_connection(&mut level, stairs_up, stairs_down)
            .unwrap();
    ...
    fn test_3d_stair_layout_generation() {
        ...
        for floor_id in 0..25 {
            ...
            assert_eq!(
                down_pos,
                up_pos_next,
                "Stairs should align between floors {} and {}",
                floor_id,
                floor_id + 1
            );
        }
    ...
    fn test_complete_dungeon_generation() {
        ...
        let world = generator
            .generate_complete_dungeon(&config, &mut rng)
            .unwrap();
        ...
        for level_id in 0..26 {
            assert!(
                world.get_level(level_id).is_some(),
                "Level {} should exist",
                level_id
            );
        }
        ...
        for level_id in 0..25 {
            ...
            if let (Some(down_pos), Some(up_pos)) = (
                ...
                assert_eq!(
                    down_pos,
                    up_pos,
                    "Stair positions should match between levels {} and {}",
                    level_id,
                    level_id + 1
                );
            }
    ...
    fn test_room_around_position() {
        ...
        let room = generator
            .create_room_around_position(1, center_pos, &config, &mut rng, &level)
            .unwrap();

        // Room should contain the center position
        assert!(
            room.contains(center_pos),
            "Room should contain the center position"
        );
    ...
    fn test_floor_0_generation_debug() {
        ...
        let floor_0_result =
            generator.generate_floor_with_stairs(0, &stair_positions, &config, &mut rng);
        ...
        match floor_0_result {
            Ok(level) => {
                ...
                let passable_count = level
                    ...
                    .count();
                println!(
                    "Floor 0 generated successfully with {} passable tiles",
                    passable_count
                );
    ...
    fn test_stair_alignment_consistency() {
        ...
        for seed_offset in 0..5 {
            let mut test_rng =
                utils::create_rng(&GenerationConfig::for_testing(44444 + seed_offset));
            let stair_positions = generator
                .generate_stair_layout(&config, &mut test_rng)
                .unwrap();
            ...
            for floor_id in 0..25 {
                ...
                assert_eq!(
                    down_current,
                    up_next,
                    "Stairs misaligned between floors {} and {}",
                    floor_id,
                    floor_id + 1
                );
            }
    ...
    fn test_room_around_position_edge_cases() {
        ...
        for pos in edge_positions {
            let room = generator
                .create_room_around_position(1, pos, &config, &mut rng, &level)
                .unwrap();
            ...
            assert!(
                room.contains(pos),
                "Room should contain target position {:?}",
                pos
            );
        }
    }
    ...
    fn test_progressive_wall_placement_3d_vs_single() {
        ...
        generator_3d
            .progressive_wall_placement(&mut level_3d, &rooms, &mut rng_3d)
            .unwrap();
        generator_single
            .progressive_wall_placement(&mut level_single, &rooms, &mut rng_single)
            .unwrap();
        ...
        assert!(
            walls_3d <= walls_single,
            "3D generation should place fewer walls: {} vs {}",
            walls_3d,
            walls_single
        );
    }
    ...
    fn test_generate_complete_dungeon_performance() {
        ...
        for level_id in 0..26 {
            ...
            assert!(
                utils::validate_level(level).is_ok(),
        ...
        println!(
            "Generated complete 26-level dungeon in {:?}",
            generation_time
        );
    }
    ...
    fn test_3d_generation_stair_connectivity() {
        ...
        for level_id in 1..25 {
            // Skip level 0 (no up stairs) and 25 (no down stairs)
            let level = world.get_level(level_id).unwrap();

            if let (Some(up_pos), Some(down_pos)) =
                (level.stairs_up_position, level.stairs_down_position)
            {
                // There should be a path between up and down stairs
                assert!(
                    generator.has_path(level, up_pos, down_pos).unwrap(),
                    "Stairs should be connected on level {}",
                    level_id
                );
            }
        }
    ...
    fn test_world_generator_error_handling() {
        ...
        assert!(
            world.is_ok(),
            "World generation should handle edge cases gracefully"
        );
