...
use crate::{ThatchResult, ThatchError, Position, EntityId, new_entity_id, config};
...
impl PlayerCharacter {
    ...
    /// 
    /// ```
    /// use thatch::{PlayerCharacter, Position};
    /// 
...
impl Entity for PlayerCharacter {
    ...
    fn handle_event(&mut self, event: &GameEvent) -> ThatchResult<Vec<GameEvent>> {
        match event {
            GameEvent::EntityDamaged { entity_id, damage, source } if *entity_id == self.id => {
            ...
            GameEvent::EntityHealed { entity_id, amount, .. } if *entity_id == self.id => {
...
mod tests {
    ...
    fn test_player_damage_event_handling() {
        ...
        let has_message = events.iter().any(|e| matches!(e, GameEvent::Message { .. }));
        assert!(has_message);
    }
