...
use crate::{ThatchResult, ThatchError, Position, EntityId, config};
    ...
    pub fn set_tile(&mut self, pos: Position, tile: Tile) -> ThatchResult<()> {
        if !self.is_valid_position(pos) {
            return Err(ThatchError::InvalidState(
                format!("Position {:?} is out of bounds for level {}x{}", 
...
impl World {
    ...
    pub fn new(seed: u64) -> Self {
        ...
        let initial_level = Level::new(0, config::DEFAULT_DUNGEON_WIDTH, config::DEFAULT_DUNGEON_HEIGHT);
