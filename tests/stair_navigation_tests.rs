...
fn test_stair_navigation_3d_dungeon() {
    ...
    let stairs_down_pos = game_state
        .world
        .current_level()
        .expect("No current level")
    ...
    assert!(
        !events.is_empty(),
        "Should have generated level change events"
    );

    // Verify player position is at stairs up on level 1
    let level_1_stairs_up = game_state
        .world
        .current_level()
        .expect("No level 1")
        .stairs_up_position
    ...
    assert_eq!(
        player_pos, level_1_stairs_up,
        "Player should be at stairs up on level 1"
    );

    // Verify stairs alignment: level 0 stairs down should match level 1 stairs up
    assert_eq!(
        stairs_down_pos, level_1_stairs_up,
        "Stairs should be aligned between levels"
    );
}
...
fn test_complete_stair_alignment() {
    ...
    for level_id in 0..26 {
        assert!(
            game_state.world.get_level(level_id).is_some(),
            "Level {} should exist",
            level_id
        );
    }
    ...
    for level_id in 0..25 {
        ...
        let next_level = game_state
            .world
            .get_level(level_id + 1)
        ...
        if let (Some(down_pos), Some(up_pos)) = (
            ...
            assert_eq!(
                down_pos,
                up_pos,
                "Stairs should align between levels {} and {}",
                level_id,
                level_id + 1
            );
        }
        ...
        if level_id == 0 {
            assert!(
                current_level.stairs_up_position.is_none(),
                "Level 0 should not have up stairs"
            );
        } else {
            assert!(
                current_level.stairs_up_position.is_some(),
                "Level {} should have up stairs",
                level_id
            );
        }
        ...
        if level_id == 24 {
            assert!(
                next_level.stairs_down_position.is_none(),
                "Level 25 should not have down stairs"
            );
        } else {
            assert!(
                current_level.stairs_down_position.is_some(),
                "Level {} should have down stairs",
                level_id
            );
        }
    }
...
fn test_stair_boundary_conditions() {
    ...
    assert!(
        result.is_ok(),
        "Using stairs up from level 0 should succeed"
    );
    assert_eq!(
        game_state.completion_state,
        thatch::GameCompletionState::EscapedEarly
    );
    ...
    game_state_25
        .world
        .change_level(25)
        .expect("Failed to change to level 25");

    let level_25_pos = game_state_25
        .world
        .current_level()
        .expect("No level 25")
        .player_spawn;
    game_state_25
        .set_entity_position(player_id_25, level_25_pos)
    ...
    game_state_25
        .set_entity_position(player_id_25, test_stairs_down_pos)
        .expect("Failed to move player");

    if let Some(level) = game_state_25.world.current_level_mut() {
        level
            .set_tile(
                test_stairs_down_pos,
                thatch::Tile::new(TileType::StairsDown),
            )
    ...
    assert!(
        result_25.is_ok(),
        "Using stairs down from level 25 should succeed"
    );
    assert_eq!(
        game_state_25.completion_state,
        thatch::GameCompletionState::CompletedDungeon
    );
}
...
fn test_3d_generation_validity() {
    ...
    for level_id in 0..26 {
        ...
        let spawn_tile = level
            .get_tile(level.player_spawn)
            .expect("Spawn position should be valid");
        assert!(
            spawn_tile.tile_type.is_passable(),
            "Spawn position should be passable on level {}",
            level_id
        );
        ...
        if level_id > 0 {
            assert!(
                level.stairs_up_position.is_some(),
                "Level {} should have up stairs",
                level_id
            );
        }
        if level_id < 25 {
            assert!(
                level.stairs_down_position.is_some(),
                "Level {} should have down stairs",
                level_id
            );
        }
